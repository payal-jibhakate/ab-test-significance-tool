...
Both campaigns saw a sharp, synchronized drop in purchases around \*\*August 17–19\*\*, independently confirmed through the EDA time-series chart and the SQL outlier-flagging query. Since both campaigns dropped together, this points to an external factor (platform issue, weekend effect, or an account-level pause) rather than a problem specific to either campaign. Recommend checking ad platform status logs or account activity for that window before drawing further conclusions from this data.



\## Statistical Confidence

\- Test used: Mann-Whitney U (chosen after Shapiro-Wilk testing showed the Test group's data was not normally distributed — a standard t-test would have violated its own assumptions)

\- Result: p = 0.958 — far from the 0.05 significance threshold

\- Effect size: Cohen's d = 0.008 (negligible) — confirms this isn't a case of "not enough data," the campaigns are genuinely performing equivalently on purchases
